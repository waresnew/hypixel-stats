...
public class HypixelApi {
    ...
    public Player setStats() throws IOException, InterruptedException {
        ...
        if (!jsonObject.has("stats")) {
            switch (game) {
                case "BEDWARS": {
                    return new BedwarsPlayer(uuid, username);
                }
                case "NORMAL_SKYWARS": {
                    return new NormalSkywarsPlayer(uuid, username);
                }
                case "INSANE_SKYWARS": {
                    return new InsaneSkywarsPlayer(uuid, username);
                }
                case "RANKED_SKYWARS": {
                    return new RankedSkywarsPlayer(uuid, username);
                }
                case "SPEED_UHC": {
                    return new SpeedUHCPlayer(uuid, username);
                }
            }
        }
        ...
        if (wholejsonObject.get("success").getAsString().equals("false") && wholejsonObject.get("cause").getAsString().replace("\"", "").equals("Invalid API key")) {
            ChatUtils.print(ChatColour.RED.getColourCode() + "Invalid API key. Please run /api new to set it.");
        } else {
            switch (game) {
                case "BEDWARS": {
